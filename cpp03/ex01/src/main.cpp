...
void	runGame(void)
{
	...
	while (1)
	{
		...
		if (attacks_loop(attacks, attacker, target) || attacker.checkEnergyLevel() || target.checkEnergyLevel())
			break ;
		attacks = randomInt(1, 3);
		if (attacker.checkEnergyLevel() || target.checkEnergyLevel() || attacks_loop(attacks, target, attacker))
			break ;
	}
}
